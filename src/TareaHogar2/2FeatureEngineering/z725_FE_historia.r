...
AgregaVarRandomForest  <- function( num.trees, max.depth, min.node.size, mtry)
{
  ...
  dataset_rf[, g_prom_90_mayor_prom_30 := ifelse(g_prom_90_mayor_prom_30, 1, 0)]
  dataset_rf[, g_esta_complicado := ifelse(g_esta_complicado, 1, 0)]
  dataset_rf[, g_es_activo_virtual := ifelse(g_es_activo_virtual, 1, 0)]
  dataset_rf[, g_es_activo_remoto := ifelse(g_es_activo_remoto, 1, 0)]
  dataset_rf[, g_usa_pago_mis_cuentas := ifelse(g_usa_pago_mis_cuentas, 1, 0)]
  dataset_rf[, g_tiene_inversiones := ifelse(g_tiene_inversiones, 1, 0)]
  dataset_rf[, g_posee_prestamos := ifelse(g_posee_prestamos, 1, 0)]
  dataset_rf[, g_tiene_saldos_negativos := ifelse(g_tiene_saldos_negativos, 1, 0)]
  dataset_rf[, g_prom_90_mayor_prom_30_lag1 := ifelse(g_prom_90_mayor_prom_30_lag1, 1, 0)]
  dataset_rf[, g_esta_complicado_lag1 := ifelse(g_esta_complicado_lag1, 1, 0)]
  dataset_rf[, g_es_activo_virtual_lag1 := ifelse(g_es_activo_virtual_lag1, 1, 0)]
  dataset_rf[, g_es_activo_remoto_lag1 := ifelse(g_es_activo_remoto_lag1, 1, 0)]
  dataset_rf[, g_usa_pago_mis_cuentas_lag1 := ifelse(g_usa_pago_mis_cuentas_lag1, 1, 0)]
  dataset_rf[, g_tiene_inversiones_lag1 := ifelse(g_tiene_inversiones_lag1, 1, 0)]
  dataset_rf[, g_posee_prestamos_lag1 := ifelse(g_posee_prestamos_lag1, 1, 0)]
  dataset_rf[, g_tiene_saldos_negativos_lag1 := ifelse(g_tiene_saldos_negativos_lag1, 1, 0)]
  
  isfac <- sapply(dataset_rf, is.factor)
  isnum <- sapply(dataset_rf, is.numeric)
